...
class DDPSolverLinearDyn(DDPSolver):
    ...
    def cost_running(self, i, x, u):                                    
        ...
        cost = 0.5*np.dot(x, np.dot(self.H_xx[i,:,:], x)) \
                ...
                + (self.underact / 2) * np.dot(np.absolute(u).T,np.absolute(u)) #... add here the running cost term for taking into account the underactuation
    ...
    def cost_running_u(self, i, x, u):
        ...
        c_u = self.lmbda * u \
         + self.underact * u #... add here the derivative w.r.t u of the running cost term for taking into account the underactuation                            
        if self.CONTROL_BOUNDS:
            pass
    ...
    def cost_running_uu(self, i, x, u):
        ...
        + self.underact #... add here the second derivative w.r.t u of the running cost term for taking into account the underactuation
        if self.CONTROL_BOUNDS:
            pass
...
if __name__=='__main__':
    ...
    import time
    from utils.robot_loaders import loadPendulum as load
